...
**Core Principle**: Tools are *stateless* wrappers around system capabilities. State is managed by the Router/Context Engine.
...
**Security Rules**:
1.  **Secret Injection**: The *Agent* injects API keys from its process environment. The *Router* never sends keys in the payload.
