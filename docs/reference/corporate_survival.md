...
**Problem**: Even after updating Registry paths, terminal windows (especially inside VS Code) often fail to "see" the new values. This happens because Windows apps cache environment variables at startup and pass the _stale_ cache to sub-processes.
