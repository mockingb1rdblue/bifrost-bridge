...
export class NetworkDriver {
  ...
  getNextDelay(): number {
    ...
    if (this.totalFailures >= NetworkDriver.MAX_TOTAL_FAILURES) {
      this.halt(
        'Maximum network failures reached. Sluagh Swarm agent is being "stupid" and will stop trying.',
      );
      return NetworkDriver.MAX_DELAY;
    }

    const expBackoff =
      NetworkDriver.BASE_DELAY * Math.pow(2, Math.min(this.consecutiveFailures, 6));
