...
export interface Job {
  ...
  type: string;
  payload: any;
  // Standardized Webhook Carriers (Vellum/Fly.io Integration)
...
export interface JobResult {
  ...
  data?: any;
  error?: string;
  // Engineering Logs / Reasoning for RAG Auditor
...
const API_KEY = config.PROXY_API_KEY;
...
async function pollWorker() {
  try {
    ...
    const { queueJob, swarmTask, taskWebhookUrl, stepWebhookUrl, crewWebhookUrl, executionId } =
    ...
    if (swarmTask) {
      ...
      await processJob({
        ...
        taskWebhookUrl: swarmTask.taskWebhookUrl || taskWebhookUrl,
        stepWebhookUrl: swarmTask.stepWebhookUrl || stepWebhookUrl,
        crewWebhookUrl: swarmTask.crewWebhookUrl || crewWebhookUrl,
        executionId: swarmTask.executionId || executionId,
        taskId: swarmTask.id,
      });
    }
  ...
  try {
    await fetch(`${ROUTER_URL}/v1/queue/complete`, {
      ...
      body: JSON.stringify({
        ...
        result: result.data,
        error: result.error,
        engineeringLog: result.engineeringLog,
