    ...
    try {
        ...
        for (const issue of issues.nodes) {
            ...
            if (attempt > 3) {
                ...
                console.error(`[LinearIngestor][pick_up] ❌ Issue ${issue.id} failed 3 dispatch attempts — permanently skipping`);
                continue;
            }
            ...
            console.log(`[LinearIngestor][pick_up] ✅ Picked up issue: ${issue.title} (Attempt ${attempt}/3) | issueId=${issue.id}`);
            ...
            if (jsonMatch) {
                try {
                    ...
                    console.error(`[LinearIngestor][parse_json] ❌ Failed to parse JSON for issue | issueId=${issue.id}`);
            ...
            if (jobType === 'unknown') {
                console.log(`[LinearIngestor][parse_job] ❌ Skipping non-swarm issue | issueId=${issue.id}`);
            ...
            if (!commentRes.success) {
                ...
                console.error(`[LinearIngestor][lock] ❌ Could not lock issue — skipping dispatch to avoid duplicate | issueId=${issue.id}`);
                continue;
            }
            ...
            if (!response.ok) {
                ...
                console.error(`[LinearIngestor][dispatch] ❌ Dispatch failed: ${response.status} | issueId=${issue.id} elapsed=${Date.now() - startTime}ms`);
            } else {
                const result = await response.json() as any;
                console.log(`[LinearIngestor][dispatch] ✅ Dispatched to router | issueId=${issue.id} jobId=${result.id} elapsed=${Date.now() - startTime}ms`);
            }
        }
