...
// .wrangler/tmp/bundle-tQNlRf/checked-fetch.js
...
var GitHubClient = class {
  ...
  async generateJWT() {
    ...
    return new SignJWT({ iss: this.config.appId }).setProtectedHeader({ alg: "RS256" }).setIssuedAt().setExpirationTime("10m").sign(privateKey);
  }
  /**
  ...
  async createBranch(owner, repo, base, name) {
  ...
  async createReviewComment(owner, repo, pullNumber, body, event) {
  ...
  async mergePullRequest(owner, repo, pullNumber, method = "squash") {
...
var FlyClient = class {
  static {
    __name(this, "FlyClient");
  }
  token;
  organizationId;
  // Optional if we just use app name
  appName;
  constructor(config) {
  ...
  async spawnMachine(region = "sea", metadata = {}) {
    ...
    const body = {
      ...
      config: {
        ...
        auto_destroy: true,
        // If supported, or we rely on the agent's idle timeout.
        env: {
          ...metadata
        },
        guest: {
...
// src/events.ts
var EventStoreClient = class {
  static {
    __name(this, "EventStoreClient");
  }
  secret;
  baseUrl;
  // e.g. http://bifrost-events.flycast:8080
  constructor(config) {
...
var JobPayloadSchema = external_exports.object({
  type: external_exports.enum(["orchestration", "ingestion", "test", "echo"]),
  priority: external_exports.number().int().min(0).max(100).optional().default(0),
  data: external_exports.record(external_exports.any()).optional().default({})
});
var JobUpdateSchema = external_exports.object({
  status: external_exports.enum(["pending", "processing", "awaiting_hitl", "completed", "failed"]).optional(),
  ...
  linearIssueId: external_exports.string().optional(),
  linearIdentifier: external_exports.string().optional()
});
var JulesTaskSchema = external_exports.object({
  issueId: external_exports.string().min(1),
  type: external_exports.enum(["edit", "create", "research", "test"]).optional().default("edit"),
  title: external_exports.string().min(1),
  description: external_exports.string().min(1),
  files: external_exports.array(external_exports.string()).optional().default([]),
  priority: external_exports.number().int().optional().default(0),
  isHighRisk: external_exports.boolean().optional().default(false)
});
var JulesTaskUpdateSchema = external_exports.object({
  taskId: external_exports.string().uuid(),
  status: external_exports.enum(["pending", "active", "completed", "failed", "paused"]),
  engineeringLog: external_exports.object({
    taskId: external_exports.string(),
    whatWasDone: external_exports.string(),
    diff: external_exports.string(),
    whatWorked: external_exports.array(external_exports.string()),
    whatDidntWork: external_exports.array(external_exports.string()),
    lessonsLearned: external_exports.array(external_exports.string())
  }).optional()
});
var LinearWebhookSchema = external_exports.object({
  action: external_exports.enum(["create", "update", "remove"]),
  type: external_exports.enum(["Issue", "Comment", "Project", "Cycle"]),
  data: external_exports.object({
    id: external_exports.string(),
    identifier: external_exports.string().optional(),
    title: external_exports.string().optional(),
    state: external_exports.object({ name: external_exports.string() }).optional(),
    body: external_exports.string().optional(),
    issueId: external_exports.string().optional()
  })
});
var GitHubActionSchema = external_exports.object({
  action: external_exports.enum(["get_pr", "review", "comment", "create_pr", "merge"]),
  ...
  content: external_exports.object({
    ...
    event: external_exports.enum(["APPROVE", "REQUEST_CHANGES", "COMMENT"]).optional(),
    method: external_exports.enum(["merge", "squash", "rebase"]).optional()
  }).optional().default({})
});
...
var PerplexityClient = class {
  ...
  async chat(messages, options = {}) {
    ...
    return {
      ...
      model: data.model,
      provider: "openai"
      // Maps to 'openai' internally as Perplexity uses their schema
    };
  }
};
...
var LLMRouter = class {
  ...
  getFallbackProvider(failed) {
    const priority = ["anthropic", "deepseek", "gemini", "openai"];
  ...
  selectProvider(request) {
    ...
    switch (request.taskType) {
      ...
      default:
        ...
        if (totalCharCount > 1e5) return "gemini";
        return "deepseek";
    }
  }
  getDefaultModel(provider, taskType) {
    switch (provider) {
      case "anthropic":
...
var RouterDO = class {
  ...
  constructor(state, env) {
    ...
    this.state.blockConcurrencyWhile(async () => {
      const stored = await this.state.storage.get("router_state");
      if (stored) {
        this.storage = {
          ...this.storage,
          ...stored,
  ...
  async logError(message2, context, error, provider) {
    console.error(`[${context}] ${message2}`, error);
  ...
  calculateHealthScore() {
    ...
    if (pendingJobs < threshold) return 1;
    if (pendingJobs < threshold * 2) return 0.5;
    return 0.1;
  }
  checkRateLimit(key) {
    const now = Date.now();
    ...
    const timePassed = (now - limitState.lastRefill) / 1e3;
  ...
  async getJulesNextTask() {
    const nextTask = Object.values(this.storage.julesTasks).filter((t) => t.status === "pending").sort((a, b) => b.priority - a.priority || a.createdAt - b.createdAt)[0];
    if (!nextTask) {
    ...
    try {
      ...
      if (status === "completed" || status === "failed") {
        try {
          ...
          const logBody = task.engineeringLog ? this.formatEngineeringLog(task.engineeringLog) : `Task ${status}`;
          await linear.addComment(task.issueId, `\u{1F916} Jules: Task ${status}
...
${logBody}`);
  ...
  async handleGitHubAction(request) {
    try {
      const body = await request.json();
      ...
      if (!this.env.GITHUB_APP_ID || !this.env.GITHUB_PRIVATE_KEY || !this.env.GITHUB_INSTALLATION_ID) {
...
${log.lessonsLearned.map((l) => `- ${l}`).join("\n")}
  ...
  async handleJobUpdate(jobId, request) {
  ...
  async processBatch() {
    ...
    const pendingJobs = Object.values(this.storage.jobs).filter((j) => j.status === "pending").sort((a, b) => b.priority - a.priority).slice(0, this.BATCH_SIZE);
    const linear = new LinearClient({
      apiKey: this.env.LINEAR_API_KEY,
      teamId: this.env.LINEAR_TEAM_ID
    });
...
Please approve this ingestion by adding a comment 'APPROVE' or updating the status.`,
          ...
          await this.logError(e.message, "LINEAR_CREATE_ISSUE", e);
          job.status = "failed";
          job.error = e.message;
        }
  ...
  async handleWebhook(request) {
    try {
      ...
      const isValid2 = await verifyLinearSignature(
        rawBody,
        signature,
  ...
  async processOrchestrationJob(job) {
    try {
      const github = new GitHubClient({
        appId: this.env.GITHUB_APP_ID,
        privateKey: this.env.GITHUB_PRIVATE_KEY,
...
+ Base: refs/heads/master`,
        ...
        const comment = `\u{1F680} **Workspace Initialized**
...
Analyze the data and suggest 3 concrete improvements to prompts, routing logic, or model selection to improve success rates. Highlight any provider that is underperforming.`;
        ...
        try {
          ...
          await linear2.createIssue({
            title: `[SWARM] Optimization Review - ${(/* @__PURE__ */ new Date()).toLocaleDateString()}`,
...
- Errors: ${metrics.errorCount}`,
  ...
  async handleQueuePoll(request) {
    try {
      const body = await request.json();
      const workerId = body.workerId;
      console.log(`Worker ${workerId} polling for work...`);
      const job = Object.values(this.storage.jobs).filter((j) => j.status === "pending").sort((a, b) => b.priority - a.priority || a.createdAt - b.createdAt)[0];
      if (!job) {
  ...
  async handleQueueComplete(request) {
    try {
      const body = await request.json();
      ...
      if (job.type === "runner_task" && job.status === "completed" && job.payload.action === "write_file") {
        ...
        const reviewJob = {
          ...
          status: "pending",
          priority: 5,
          // Lower priority than original work
          topic: job.topic,
  ...
  async maybeTriggerOptimization() {
    const errorRate = this.storage.metrics.totalRequests > 0 ? this.storage.metrics.errorCount / this.storage.metrics.totalRequests : 0;
  ...
  async syncLinearTasks() {
    try {
      ...
      const readyLabel = labels.find((l) => l.name === "swarm:ready");
      const activeLabel = labels.find((l) => l.name === "swarm:active");
      ...
      for (const issue of issues) {
        ...
        const otherLabels = (issue.labels.nodes || []).filter((l) => l.name !== "swarm:ready").map((l) => l.id);
...
var src_default = {
  async fetch(request, env) {
    ...
    if (url.pathname === "/webhooks/linear") {
      ...
      const id2 = env.ROUTER_DO.idFromName("global-router");
      const obj2 = env.ROUTER_DO.get(id2);
      return await obj2.fetch(request);
    }
...
// .wrangler/tmp/bundle-tQNlRf/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-tQNlRf/middleware-loader.entry.ts
