...
export class GovernanceDO extends DurableObject {
  ...
  private async handleCheck(request: Request): Promise<Response> {
    if (this.state.blocked) {
      return new Response(JSON.stringify({ allowed: false, reason: 'Daily limit exceeded' }), {
        status: 429,
      });
    }
    ...
    return new Response(JSON.stringify({ allowed: true, count: this.state.requestsToday }), {
      status: 200,
    });
  }
  ...
  private scheduleNextReset() {
    ...
    console.log(
      `[GovernanceDO] ⏰ Next reset scheduled in ${Math.floor(timeUntil / 1000 / 60)} minutes.`,
    );
  }
