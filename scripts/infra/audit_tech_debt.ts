...
const TARGETS = [
  ...
  { name: 'Workers', path: 'workers', recursive: true, depth: 2 }, // Audit each worker
  {
    ...
    path: 'docs',
    recursive: false,
    files: ['OPERATIONAL_MANUAL.md', 'SYSTEM_ARCHITECTURE.md'],
  },
];
...
async function scanDirectory(
  ...
  depth: number = 1,
  checkSecrets = false,
): Promise<{ context: string; secrets: string[] }> {
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (item === 'node_modules' || item.startsWith('.') || item === 'dist' || item === 'logs')
        continue;
...
async function main() {
  ...
  if (fs.existsSync(workersDir)) {
    const workers = fs
      ...
      .filter((f) => fs.statSync(path.join(workersDir, f)).isDirectory());
