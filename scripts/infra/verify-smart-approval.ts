...
async function main() {
  ...
  if (createdTask.id !== taskId) {
    console.warn(
      `⚠️ Warning: Router returned different ID (${createdTask.id}) than requested (${taskId}). Using returned ID.`,
    );
  ...
  while (retries > 0) {
    ...
    if (resp.ok) {
      ...
      if (
        task.type === 'verify' &&
        (task.id === `task_verify_${taskId}` ||
          task.title.includes(taskId) ||
          task.description.includes(taskId) ||
