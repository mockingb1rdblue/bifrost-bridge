...
async function main() {
  ...
  if (issuesToInspect.length === 0) {
    console.log(
      chalk.yellow(
        '⚠️ No issue identifiers provided. Usage: npx ts-node scripts/inspect_issue_details.ts BIF-104 BIF-105 [--direct]',
      ),
    );
    process.exit(0);
  }
  ...
  for (const identifier of issuesToInspect) {
    ...
    try {
      ...
      console.log(
        chalk.gray(`Labels: ${issue.labels.nodes.map((l: any) => l.name).join(', ') || 'None'}`),
      );
