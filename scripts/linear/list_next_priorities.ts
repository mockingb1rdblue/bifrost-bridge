...
async function main() {
    ...
    const activeIssues = allIssues.filter(issue =>
        issue.status.type !== 'completed' && issue.status.type !== 'canceled'
    );
    ...
    activeIssues.forEach(issue => {
        ...
        const statusLabel = issue.status.type === 'started' ? chalk.bgGreen.black(` ${issue.status.name} `) : chalk.bgBlue.white(` ${issue.status.name} `);

        console.log(`${chalk.bold(issue.identifier)} ${statusLabel} ${priorityLabel} ${chalk.white(issue.title)} ${chalk.gray(`(${issue.projectName})`)}`);
        if (issue.assignee) {
